...
class TestQAT(TestCase):
    ...
    @parametrize(
        "base_config_cls", [IntxWeightOnlyConfig, Int8DynamicActivationIntxWeightConfig]
    )
    def test_range_learning_convert_pass_qparams(
        ...
        base_config_args = (torch.int4, PerGroup(group_size))
        base_config = base_config_cls(*base_config_args)
        quantize_(m, QATConfig(base_config, step="convert"))
